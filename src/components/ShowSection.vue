...
export default {
  ...
  setup: (props: { genre: string; shows: Show[]; showSize: number }) => {
    const index = ref(0);
...
<template>
  <div class="flex flex-col overflow-x-auto p-2">
    ...
    <div class="flex flex-row gap-5 p-5 justify-start">
      <div
        @click="previous"
        v-if="index > 0"
      ...
      />
      <div
        v-if="slicedShows.length == showSize && shows.length > showSize"
        @click="next"
